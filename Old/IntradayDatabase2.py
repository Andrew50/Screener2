...
class Data:
    ...
    def updatTick(tickersString):
        ...
        try:
            ...
            for t in tickers:
                ...
                if(os.path.exists("C:/Screener/intraday_data/" + tick + ".csv") == False):
                    ticker_df.to_csv("C:/Screener/intraday_data/" + tick + ".csv")
                    print(f"created {tick}")
                else:
                    cs = pd.read_csv(r"C:/Screener/intraday_data/" + tick + ".csv")
                    ...
                    if scrapped_data_index != -1:
                        ...
                        cs.to_csv("C:/Screener/intraday_data/" + tick + ".csv")
    ...
    def isTickerUpdated(tickerandDate):
        ...
        if(os.path.exists("C:/Screener/intraday_data/" + ticker + ".csv") == False):
            s = f"{ticker}:{exchange}"
            return s
        else:
            
            cs = pd.read_csv(r"C:/Screener/intraday_data/" + ticker + ".csv")
            if len(cs) > 1:
